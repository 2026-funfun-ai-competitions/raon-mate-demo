...
class WorkshopPlannerFlowTests {
 ...
 @Test void completesDemoPlannerFlow(){
  ...
  assertEquals("가평",updated.departureLocation());
  ...
  var budget=budgets.initialize(w.id()); assertEquals(0,new BigDecimal("3750000").compareTo(budget.limitAmount())); assertEquals(0,BigDecimal.ZERO.compareTo(budget.remainingAmount()));
